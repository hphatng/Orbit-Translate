...
import { ArrowLeft, Mail, Lock, User, Eye, EyeOff, Sparkles, CheckCircle2, ArrowRight, AlertCircle, MailCheck } from 'lucide-react';
...
function SignupPageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isConfigured) {
        ...
        if (error) {
          if (error.message.toLowerCase().includes('failed to fetch')) {
            setErrorMessage('Không thể kết nối đến Supabase URL. Vui lòng kiểm tra lại URL trong web/.env.local.');
          } else {
        ...
        if (data?.user) {
          ...
          if (data.session) {
            ...
            setIsLoading(false);
            setSuccessMessage(`Đã tạo tài khoản cho ${email}! Vui lòng kiểm tra hộp thư email để kích hoạt tài khoản.`);
          }
          return;
        }
      }
      ...
      if (msg.toLowerCase().includes('failed to fetch')) {
        setErrorMessage('Không thể kết nối đến Supabase. Hãy kiểm tra lại kết nối mạng hoặc Supabase URL trong web/.env.local.');
      } else {
        setErrorMessage(msg || 'Có lỗi xảy ra khi kết nối Supabase.');
      }
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0B0F17] text-white flex flex-col justify-between relative overflow-hidden font-sans">
      ...
      <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[400px] bg-emerald-600/10 rounded-full blur-[140px] pointer-events-none" />
        ...
        >
          <div className="card-obsidian p-8 sm:p-10 rounded-3xl border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.85)] bg-[#131722]/95 backdrop-blur-2xl">
            ...
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono-data font-bold mb-3">
                <Sparkles className="w-3 h-3" /> Free 100% Forever
              </div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-white font-heading tracking-tight mb-2">
                Tạo Tài Khoản Mới
              </h1>
              <p className="text-xs sm:text-sm text-gray-400">
                Bắt đầu lưu &amp; ôn tập từ vựng bằng thuật toán FSRS
              </p>
            </div>
            ...
            {errorMessage && (
              <div className="mb-4 p-3.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-mono-data flex items-start gap-2 leading-relaxed">
            ...
            {successMessage && (
              <div className="mb-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-mono-data leading-relaxed flex items-start gap-2.5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5 pt-2 text-xs text-gray-400 font-mono-data">
                <div className="flex items-center gap-2 text-gray-300">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Tự động đồng bộ với Chrome Extension
                </div>
                <div className="flex items-center gap-2 text-gray-300">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Thuật toán FSRS chuẩn khoa học
                </div>
              </div>
            ...
            <div className="mt-8 text-center text-xs text-gray-400">
              Đã có tài khoản Orbit Translate?{' '}
              <Link href={`/login${searchParams.get('next') ? `?next=${encodeURIComponent(searchParams.get('next')!)}` : ''}`} className="text-indigo-400 font-bold hover:underline ml-1">
                Đăng nhập
              </Link>
            </div>
